...
def train(RemoteEmulator, PackedEmulator, missing_samples=None, cfg=None):
    ...
    n_samples = len(training_data) // 2
    training_data.inputs = training_data.inputs.isel(sample=slice(n_samples))
    training_data.targets = training_data.targets.isel(sample=slice(n_samples))
